...
func ClassifyStream(r io.Reader) (sc StreamClassification, out io.Reader, err error) {
	// This is currently the default size. It is specified explicitly because go 1.9 does not
	// expose the size of a bufio.Reader (go 1.10 does).
	stream := bufio.NewReaderSize(r, 4096)

	buf, err := stream.Peek(4096)
	if err != nil {
		// If we had a short peek (for example, due to a short stream), we can still continue
		if len(buf) == 0 {
	...
	case isSaltpackMessage(peeker, &sc):
